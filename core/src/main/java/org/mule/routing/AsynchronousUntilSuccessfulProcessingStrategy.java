...
public class AsynchronousUntilSuccessfulProcessingStrategy extends AbstractUntilSuccessfulProcessingStrategy implements Initialisable, Startable, Stoppable
{
    ...
    public static final String TOC_LOG_MESSAGE_ID_PROPERTY_NANE = "LOG_MESSAGE_ID";
    ...
    private void incrementProcessAttemptCountAndRescheduleOrRemoveFromStore(final Serializable eventStoreKey) throws Exception
    {
        try
        {
             ...
             if (deliveryAttemptCount <= getUntilSuccessfulConfiguration().getMaxRetries())
            {
                ...
                // been reached
                logger.debug("For event stored with key " + eventStoreKey + " deliveryAttemptCount=" + deliveryAttemptCount);
            ...
            {
                logger.info("For event stored with key " + eventStoreKey + " has reached maximum delivery attempts. Will abandon retries");
        ...
        catch (final ObjectStoreException ose)
        {
            ...
            logger.warn("Error for event stored with key " + eventStoreKey + ose.getMessage());
        }
    }
    ...
    public static Serializable buildQueueKey(final MuleEvent muleEvent)
    {
        ...
        String key = muleEvent.getFlowConstruct() + "-" + muleEvent.getMuleContext().getClusterId() + "-"
                     + muleEvent.getId();
        try {
            String logId = muleEvent.getMessage().findPropertyInAnyScope(TOC_LOG_MESSAGE_ID_PROPERTY_NANE, null);
            if(logId != null) {
                key = key + "--LogId-" + logId;
            }
            if(muleEvent.getMessage().getCorrelationSequence() > 0) {
                key = key + "-" + muleEvent.getMessage().getCorrelationSequence();
            }
    ...
    private void abandonRetries(final MuleEvent event, final MuleEvent mutableEvent)
    {
        ...
        {
            logger.warn("Retry attempts exhausted and no DLQ defined");
        ...
        logger.debug("Retry attempts exhausted, routing message to DLQ: " + getUntilSuccessfulConfiguration().getDlqMP());
        try
        {
        ...
        {
            logger.warn("Could not route to DLQ" + e.getMessage());
            event.getFlowConstruct()
                    .getExceptionListener()
    ...
    private void removeFromStore(final Serializable eventStoreKey)
    {
        try
        {
            logger.info("Removing " + eventStoreKey + " from objectStore");
